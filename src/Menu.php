    ...
    public function render()
    {
        ...
        $checkUrl = function ($item) use ($url) {
            if (!$item->IsCustomUrl && CurrentPageName() == GetPageName($item->Url) || $item->IsCustomUrl && $url == GetUrl($item->Url)) { // Active
				// Begin of modification Masino Sinaga, December 16, 2021
				$item->Active = true;
				...
				// End of modification Masino Sinaga, December 16, 2021
				...
				// End of modification Masino Sinaga, December 16, 2021
                $item->setAttribute("data-ew-action", "none");
            } else  { // Begin of modification Masino Sinaga, December 16, 2021
				if ($item->IsJsMenu == TRUE) {
					$item->setAttribute("onclick", $item->OnClick);
				}
            } // End of modification Masino Sinaga, December 16, 2021
