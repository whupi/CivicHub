...
// Page object
$SubmissionMonitorEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { submission_monitor: currentTable } });
var currentForm, currentPageID;
var fsubmission_monitoredit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsubmission_monitoredit = new ew.Form("fsubmission_monitoredit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsubmission_monitoredit;

    // Add fields
    var fields = currentTable.fields;
    fsubmission_monitoredit.addFields([
    ...
    // Form_CustomValidate
    fsubmission_monitoredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubmission_monitoredit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsubmission_monitoredit.lists.Submission_ID = <?= $Page->Submission_ID->toClientList($Page) ?>;
    fsubmission_monitoredit.lists.Status = <?= $Page->Status->toClientList($Page) ?>;
    fsubmission_monitoredit.lists.Organisations = <?= $Page->Organisations->toClientList($Page) ?>;
    loadjs.done("fsubmission_monitoredit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsubmission_monitoredit" id="fsubmission_monitoredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->Monitor_ID->Visible) { // Monitor_ID ?>
    <div id="r_Monitor_ID"<?= $Page->Monitor_ID->rowAttributes() ?>>
        <label id="elh_submission_monitor_Monitor_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Monitor_ID->caption() ?><?= $Page->Monitor_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Monitor_ID->cellAttributes() ?>>
<span id="el_submission_monitor_Monitor_ID">
<span<?= $Page->Monitor_ID->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Monitor_ID->getDisplayValue($Page->Monitor_ID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="submission_monitor" data-field="x_Monitor_ID" data-hidden="1" name="x_Monitor_ID" id="x_Monitor_ID" value="<?= HtmlEncode($Page->Monitor_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_submission_monitor_Submission_ID">
    <select
        ...
        data-select2-id="fsubmission_monitoredit_x_Submission_ID"
...
<script>
loadjs.ready("fsubmission_monitoredit", function() {
    var options = { name: "x_Submission_ID", selectId: "fsubmission_monitoredit_x_Submission_ID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_monitoredit.lists.Submission_ID.lookupOptions.length) {
        options.data = { id: "x_Submission_ID", form: "fsubmission_monitoredit" };
    } else {
        options.ajax = { id: "x_Submission_ID", form: "fsubmission_monitoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_submission_monitor_Status">
    <select
        ...
        data-select2-id="fsubmission_monitoredit_x_Status"
        data-table="submission_monitor"
        data-field="x_Status"
...
<script>
loadjs.ready("fsubmission_monitoredit", function() {
    var options = { name: "x_Status", selectId: "fsubmission_monitoredit_x_Status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_monitoredit.lists.Status.lookupOptions.length) {
        options.data = { id: "x_Status", form: "fsubmission_monitoredit" };
    } else {
        options.ajax = { id: "x_Status", form: "fsubmission_monitoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsubmission_monitoredit", "editor"], function() {
    ew.createEditor("fsubmission_monitoredit", "x_Taskings", 0, 0, <?= $Page->Taskings->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fsubmission_monitoredit_x_Organisations[]"
        data-table="submission_monitor"
...
<script>
loadjs.ready("fsubmission_monitoredit", function() {
    var options = { name: "x_Organisations[]", selectId: "fsubmission_monitoredit_x_Organisations[]" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.multiple = true;
    options.closeOnSelect = false;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_monitoredit.lists.Organisations.lookupOptions.length) {
        options.data = { id: "x_Organisations[]", form: "fsubmission_monitoredit" };
    } else {
        options.ajax = { id: "x_Organisations[]", form: "fsubmission_monitoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsubmission_monitoredit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsubmission_monitoredit", "x_Start_Date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fsubmission_monitoredit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsubmission_monitoredit", "x_Finish_Date", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
