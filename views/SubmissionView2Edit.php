...
// Page object
$SubmissionView2Edit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { submission_view2: currentTable } });
var currentForm, currentPageID;
var fsubmission_view2edit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsubmission_view2edit = new ew.Form("fsubmission_view2edit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsubmission_view2edit;

    // Add fields
    var fields = currentTable.fields;
    fsubmission_view2edit.addFields([
    ...
    // Form_CustomValidate
    fsubmission_view2edit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubmission_view2edit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsubmission_view2edit.lists.Category_ID = <?= $Page->Category_ID->toClientList($Page) ?>;
    fsubmission_view2edit.lists.Status = <?= $Page->Status->toClientList($Page) ?>;
    fsubmission_view2edit.lists.Tags = <?= $Page->Tags->toClientList($Page) ?>;
    loadjs.done("fsubmission_view2edit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsubmission_view2edit" id="fsubmission_view2edit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->Submission_ID->Visible) { // Submission_ID ?>
    <div id="r_Submission_ID"<?= $Page->Submission_ID->rowAttributes() ?>>
        <label id="elh_submission_view2_Submission_ID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Submission_ID->caption() ?><?= $Page->Submission_ID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Submission_ID->getDisplayValue($Page->Submission_ID->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="submission_view2" data-field="x_Submission_ID" data-hidden="1" name="x_Submission_ID" id="x_Submission_ID" value="<?= HtmlEncode($Page->Submission_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fsubmission_view2edit_x_Category_ID"
...
<script>
loadjs.ready("fsubmission_view2edit", function() {
    var options = { name: "x_Category_ID", selectId: "fsubmission_view2edit_x_Category_ID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_view2edit.lists.Category_ID.lookupOptions.length) {
        options.data = { id: "x_Category_ID", form: "fsubmission_view2edit" };
    } else {
        options.ajax = { id: "x_Category_ID", form: "fsubmission_view2edit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_submission_view2_Status">
    <select
        ...
        data-select2-id="fsubmission_view2edit_x_Status"
        data-table="submission_view2"
        data-field="x_Status"
...
<script>
loadjs.ready("fsubmission_view2edit", function() {
    var options = { name: "x_Status", selectId: "fsubmission_view2edit_x_Status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_view2edit.lists.Status.lookupOptions.length) {
        options.data = { id: "x_Status", form: "fsubmission_view2edit" };
    } else {
        options.ajax = { id: "x_Status", form: "fsubmission_view2edit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsubmission_view2edit", "editor"], function() {
    ew.createEditor("fsubmission_view2edit", "x__Abstract", 60, 4, <?= $Page->_Abstract->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_submission_view2_Tags">
    <select
        ...
        data-select2-id="fsubmission_view2edit_x_Tags[]"
        data-table="submission_view2"
        data-field="x_Tags"
        multiple
        size="1"
...
<script>
loadjs.ready("fsubmission_view2edit", function() {
    var options = { name: "x_Tags[]", selectId: "fsubmission_view2edit_x_Tags[]" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.multiple = true;
    options.closeOnSelect = false;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_view2edit.lists.Tags.lookupOptions.length) {
        options.data = { id: "x_Tags[]", form: "fsubmission_view2edit" };
    } else {
        options.ajax = { id: "x_Tags[]", form: "fsubmission_view2edit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<input type="hidden" name="fa_x_Cover" id= "fa_x_Cover" value="<?= (Post("fa_x_Cover") == "0") ? "0" : "1" ?>">
