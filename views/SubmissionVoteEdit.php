...
// Page object
$SubmissionVoteEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { submission_vote: currentTable } });
var currentForm, currentPageID;
var fsubmission_voteedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fsubmission_voteedit = new ew.Form("fsubmission_voteedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fsubmission_voteedit;

    // Add fields
    var fields = currentTable.fields;
    fsubmission_voteedit.addFields([
    ...
    // Form_CustomValidate
    fsubmission_voteedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubmission_voteedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fsubmission_voteedit.lists.Submission_ID = <?= $Page->Submission_ID->toClientList($Page) ?>;
    fsubmission_voteedit.lists.Vote = <?= $Page->Vote->toClientList($Page) ?>;
    loadjs.done("fsubmission_voteedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fsubmission_voteedit" id="fsubmission_voteedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span class="form-control-plaintext"><?= $Page->Submission_ID->getDisplayValue($Page->Submission_ID->EditValue) ?></span></span>
</span>
<input type="hidden" id="x_Submission_ID" name="x_Submission_ID" value="<?= HtmlEncode($Page->Submission_ID->CurrentValue) ?>" data-hidden="1">
<?php } else { ?>
    <select
        id="x_Submission_ID"
        name="x_Submission_ID"
        class="form-select ew-select<?= $Page->Submission_ID->isInvalidClass() ?>"
        data-select2-id="fsubmission_voteedit_x_Submission_ID"
...
<script>
loadjs.ready("fsubmission_voteedit", function() {
    var options = { name: "x_Submission_ID", selectId: "fsubmission_voteedit_x_Submission_ID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsubmission_voteedit.lists.Submission_ID.lookupOptions.length) {
        options.data = { id: "x_Submission_ID", form: "fsubmission_voteedit" };
    } else {
        options.ajax = { id: "x_Submission_ID", form: "fsubmission_voteedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
<input type="hidden" data-table="submission_vote" data-field="x_Submission_ID" data-hidden="1" name="o_Submission_ID" id="o_Submission_ID" value="<?= HtmlEncode($Page->Submission_ID->OldValue ?? $Page->Submission_ID->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<?php } ?>
    ...
    <input type="hidden" data-table="submission_vote" data-field="x_Updated_Username" data-hidden="1" name="x_Updated_Username" id="x_Updated_Username" value="<?= HtmlEncode($Page->Updated_Username->CurrentValue) ?>">
<?php if (!$Page->IsModal) { ?>
