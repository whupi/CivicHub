...
var fref_categoryadd;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fref_categoryadd = new ew.Form("fref_categoryadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fref_categoryadd;

    // Add fields
    var fields = currentTable.fields;
    fref_categoryadd.addFields([
    ...
    // Form_CustomValidate
    fref_categoryadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fref_categoryadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("fref_categoryadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fref_categoryadd" id="fref_categoryadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fref_categoryadd", "editor"], function() {
    ew.createEditor("fref_categoryadd", "x_Category_Description", 0, 0, <?= $Page->Category_Description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<?php
    if (in_array("submission", explode(",", $Page->getCurrentDetailTable())) && $submission->DetailAdd) {
?>
