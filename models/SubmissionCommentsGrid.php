 ...
 * Page class
 */
class SubmissionCommentsGrid extends SubmissionComments
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'submission_comments';

    // Page object name
    public $PageObjName = "SubmissionCommentsGrid";
    ...
    public $FormName = "fsubmission_commentsgrid";
    ...
    public function __construct()
    {
        ...
        // Table object (submission_comments)
        if (!isset($GLOBALS["submission_comments"]) || get_class($GLOBALS["submission_comments"]) == PROJECT_NAMESPACE . "submission_comments") {
            $GLOBALS["submission_comments"] = &$this;
        }
        $this->AddUrl = "submissioncommentsadd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'submission_comments');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("submission_comments");
                $doc = new $class($tbl);
                $doc->Text = @$content;
...
// Use layout
        ...
        $this->setupLookupOptions($this->Updated_Username);
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Comment_ID->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_Comment") && $CurrentForm->hasValue("o_Comment") && $this->Comment->CurrentValue != $this->Comment->DefaultValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->Comment->clearErrorMessage();
    }
...
// Get upload files
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Comment_ID' first before field var 'x_Comment_ID'
        $val = $CurrentForm->hasValue("Comment_ID") ? $CurrentForm->getValue("Comment_ID") : $CurrentForm->getValue("x_Comment_ID");
        if (!$this->Comment_ID->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Comment_ID->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Comment_ID->CurrentValue = $this->Comment_ID->FormValue;
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->Comment_ID->setDbValue($row['Comment_ID']);
        $this->Submission_ID->setDbValue($row['Submission_ID']);
        $this->Updated_Username->setDbValue($row['Updated_Username']);
        if (array_key_exists('EV__Updated_Username', $row)) {
            $this->Updated_Username->VirtualValue = $row['EV__Updated_Username']; // Set up virtual field value
        } else {
            $this->Updated_Username->VirtualValue = ""; // Clear value
        }
    ...
    {
        $row = [];
        $row['Comment_ID'] = $this->Comment_ID->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // Comment_ID
            $this->Comment_ID->ViewValue = $this->Comment_ID->CurrentValue;
            $this->Comment_ID->ViewValue = FormatNumber($this->Comment_ID->ViewValue, $this->Comment_ID->formatPattern());
            $this->Comment_ID->ViewCustomAttributes = "";
            ...
            // Updated_Username
            if ($this->Updated_Username->VirtualValue != "") {
                $this->Updated_Username->ViewValue = $this->Updated_Username->VirtualValue;
            } else {
                $curVal = strval($this->Updated_Username->CurrentValue);
                if ($curVal != "") {
                    $this->Updated_Username->ViewValue = $this->Updated_Username->lookupCacheOption($curVal);
                    if ($this->Updated_Username->ViewValue === null) { // Lookup from database
                        $filterWrk = "`Username`" . SearchString("=", $curVal, DATATYPE_STRING, "");
                        $sqlWrk = $this->Updated_Username->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                        $conn = Conn();
                        $config = $conn->getConfiguration();
                        $config->setResultCacheImpl($this->Cache);
                        $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                        $ari = count($rswrk);
                        if ($ari > 0) { // Lookup values found
                            $arwrk = $this->Updated_Username->Lookup->renderViewRow($rswrk[0]);
                            $this->Updated_Username->ViewValue = $this->Updated_Username->displayValue($arwrk);
                        } else {
                            $this->Updated_Username->ViewValue = $this->Updated_Username->CurrentValue;
                        }
                    }
                } else {
                    $this->Updated_Username->ViewValue = null;
                }
            }
    ...
    protected function editRow()
    {
        ...
        $this->Submission_ID->setDbValueDef($rsnew, $this->Submission_ID->CurrentValue, null, $this->Submission_ID->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Submission_ID
        $this->Submission_ID->setDbValueDef($rsnew, $this->Submission_ID->CurrentValue, null, false);
