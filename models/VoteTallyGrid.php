 ...
 * Page class
 */
class VoteTallyGrid extends VoteTally
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VoteTallyGrid";
    ...
    public $FormName = "fvote_tallygrid";
...
// Use layout
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_Votes") && $CurrentForm->hasValue("o_Votes") && $this->Votes->CurrentValue != $this->Votes->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_Count") && $CurrentForm->hasValue("o_Count") && $this->Count->CurrentValue != $this->Count->DefaultValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->Votes->clearErrorMessage();
        $this->Count->clearErrorMessage();
    }
...
// Get upload files
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Submission_ID->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->Submission_ID->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'Votes' first before field var 'x_Votes'
        $val = $CurrentForm->hasValue("Votes") ? $CurrentForm->getValue("Votes") : $CurrentForm->getValue("x_Votes");
        if (!$this->Votes->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Votes->Visible = false; // Disable update for API request
            } else {
                $this->Votes->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_Votes")) {
            $this->Votes->setOldValue($CurrentForm->getValue("o_Votes"));
        }
        ...
        if ($CurrentForm->hasValue("o_Count")) {
            $this->Count->setOldValue($CurrentForm->getValue("o_Count"));
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        $this->Votes->CurrentValue = $this->Votes->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->Submission_ID->getSessionValue() != "") {
                ...
                $this->Submission_ID->EditValue = HtmlEncode($this->Submission_ID->CurrentValue);
                $this->Submission_ID->PlaceHolder = RemoveHtml($this->Submission_ID->caption());
                if (strval($this->Submission_ID->EditValue) != "" && is_numeric($this->Submission_ID->EditValue)) {
                    $this->Submission_ID->EditValue = FormatNumber($this->Submission_ID->EditValue, null);
                    $this->Submission_ID->OldValue = $this->Submission_ID->EditValue;
                }
            }
            ...
            // Votes
            $this->Votes->EditCustomAttributes = "";
            $this->Votes->EditValue = $this->Votes->options(false);
            $this->Votes->PlaceHolder = RemoveHtml($this->Votes->caption());
            ...
            if (strval($this->Count->EditValue) != "" && is_numeric($this->Count->EditValue)) {
                ...
                $this->Count->OldValue = $this->Count->EditValue;
            }
    ...
    protected function validateForm()
    {
        ...
        if (!CheckInteger($this->Submission_ID->FormValue)) {
            $this->Submission_ID->addErrorMessage($this->Submission_ID->getErrorMessage(false));
        }
        ...
        if ($this->Votes->Required) {
            if ($this->Votes->FormValue == "") {
                $this->Votes->addErrorMessage(str_replace("%s", $this->Votes->caption(), $this->Votes->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // Votes
        $this->Votes->setDbValueDef($rsnew, $this->Votes->CurrentValue, null, $this->Votes->ReadOnly);
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Votes
        $this->Votes->setDbValueDef($rsnew, $this->Votes->CurrentValue, null, false);

        // Count
        $this->Count->setDbValueDef($rsnew, $this->Count->CurrentValue, 0, strval($this->Count->CurrentValue ?? "") == "");
