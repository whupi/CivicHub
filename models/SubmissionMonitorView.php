 ...
 * Page class
 */
class SubmissionMonitorView extends SubmissionMonitor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubmissionMonitorView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Monitor_ID") ?? Route("Monitor_ID")) !== null) {
            $this->RecKey["Monitor_ID"] = $keyValue;
        }
...
// Is modal
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("Monitor_ID") ?? Route("Monitor_ID")) !== null) {
                ...
                $this->RecKey["Monitor_ID"] = $this->Monitor_ID->QueryStringValue;
            } elseif (Post("Monitor_ID") !== null) {
                $this->Monitor_ID->setFormValue(Post("Monitor_ID"));
                $this->RecKey["Monitor_ID"] = $this->Monitor_ID->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->Monitor_ID->setQueryStringValue($keyValue);
                $this->RecKey["Monitor_ID"] = $this->Monitor_ID->QueryStringValue;
            } elseif (!$loadCurrentRecord) {
                $returnUrl = "submissionmonitorlist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "submissionmonitorlist"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "submissionmonitorlist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
