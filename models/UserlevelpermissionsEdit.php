...
class UserlevelpermissionsEdit extends Userlevelpermissions
{
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("userlevelpermissions");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['User_Level_ID'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['Table_Name'];
        }
        return $key;
    }
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("User_Level_ID") ?? Key(0) ?? Route(2)) !== null) {
                $this->User_Level_ID->setQueryStringValue($keyValue);
                $this->User_Level_ID->setOldValue($this->User_Level_ID->QueryStringValue);
            } elseif (Post("User_Level_ID") !== null) {
                $this->User_Level_ID->setFormValue(Post("User_Level_ID"));
                $this->User_Level_ID->setOldValue($this->User_Level_ID->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("Table_Name") ?? Key(1) ?? Route(3)) !== null) {
                $this->Table_Name->setQueryStringValue($keyValue);
                $this->Table_Name->setOldValue($this->Table_Name->QueryStringValue);
            } elseif (Post("Table_Name") !== null) {
                $this->Table_Name->setFormValue(Post("Table_Name"));
                $this->Table_Name->setOldValue($this->Table_Name->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if (Post("action") !== null) {
                ...
                if (($keyValue = Get("Table_Name") ?? Route("Table_Name")) !== null) {
                    $this->Table_Name->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
...
// Get upload files
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_Table_Name")) {
            $this->Table_Name->setOldValue($CurrentForm->getValue("o_Table_Name"));
        }
    ...
    protected function newRow()
    {
        ...
        $row['Table_Name'] = $this->Table_Name->DefaultValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->Table_Name->Required) {
            if (!$this->Table_Name->IsDetailKey && EmptyValue($this->Table_Name->FormValue)) {
                $this->Table_Name->addErrorMessage(str_replace("%s", $this->Table_Name->caption(), $this->Table_Name->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // Table_Name
        $this->Table_Name->setDbValueDef($rsnew, $this->Table_Name->CurrentValue, "", $this->Table_Name->ReadOnly);
