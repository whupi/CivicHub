...
// Use layout
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fuserlevelssrch");
        }
        $filterList = Concat($filterList, $this->User_Level_ID->AdvancedSearch->toJson(), ","); // Field User_Level_ID
        $filterList = Concat($filterList, $this->User_Level_Name->AdvancedSearch->toJson(), ","); // Field User_Level_Name
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->User_Level_Name;
    ...
    public function renderListOptions()
    {
        ...
        if ($Security->allowList(CurrentProjectID() . 'users')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("userslist?" . Config("TABLE_SHOW_MASTER") . "=userlevels&" . GetForeignKeyUrl("fk_User_Level_ID", $this->User_Level_ID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("UsersGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'userlevels')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'userlevels')) {
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'userlevels')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                ...
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'userlevels') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
