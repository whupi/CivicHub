 ...
 * Page class
 */
class VotingSummary extends Voting
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "VotingSummary";
    ...
    public function __construct()
    {
        ...
        // Table object (Voting)
        if (!isset($GLOBALS["Voting"]) || get_class($GLOBALS["Voting"]) == PROJECT_NAMESPACE . "Voting") {
            $GLOBALS["Voting"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Voting');
        }
...
// Use layout
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->Submission_ID->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->Submission_ID->setGroupValue("");
        }
    }
    ...
    {
        $data = [];
        $data["Submission_ID"] = $record['Submission_ID'];
        $data["_Title"] = $record['Title'];
        $data["_Votes"] = $record['Votes'];
        $data["Count"] = $record['Count'];
        $this->Rows[] = $data;
        $this->Submission_ID->setDbValue(GroupValue($this->Submission_ID, $record['Submission_ID']));
        $this->_Title->setDbValue($record['Title']);
        $this->_Votes->setDbValue($record['Votes']);
        $this->Count->setDbValue($record['Count']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_TOTAL && $this->RowTotalSubType == ROWTOTAL_FOOTER && $this->RowTotalType == ROWTOTAL_PAGE) {
            // Build detail SQL
            $firstGrpFld = &$this->Submission_ID;
            ...
            if ($rsagg) {
                $this->_Title->Count = $this->TotalCount;
                $this->Count->Count = $this->TotalCount;
                $this->Count->CntValue = $rsagg["cnt_count"];
                $hasSummary = true;
            }
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->Count->getCnt($this->DetailRecords);
            }
        }
        ...
        if ($this->RowType == ROWTYPE_SEARCH) { // Search row
            ...
            if ($this->RowTotalType == ROWTOTAL_GROUP) {
                $this->RowAttrs["data-group"] = $this->Submission_ID->groupValue(); // Set up group attribute
            }
            if ($this->RowTotalType == ROWTOTAL_GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->_Votes->groupValue(); // Set up group attribute 2
            }

            // Submission_ID
            $this->Submission_ID->GroupViewValue = $this->Submission_ID->groupValue();
            $this->Submission_ID->GroupViewValue = FormatNumber($this->Submission_ID->GroupViewValue, $this->Submission_ID->formatPattern());
            $this->Submission_ID->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->Submission_ID->ViewCustomAttributes = "";
            $this->Submission_ID->GroupViewValue = DisplayGroupValue($this->Submission_ID, $this->Submission_ID->GroupViewValue);

            // Votes
            if (strval($this->_Votes->groupValue()) != "") {
                $this->_Votes->GroupViewValue = $this->_Votes->optionCaption($this->_Votes->groupValue());
            } else {
                $this->_Votes->GroupViewValue = null;
            }
            $this->_Votes->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->_Votes->ViewCustomAttributes = "";
            $this->_Votes->GroupViewValue = DisplayGroupValue($this->_Votes, $this->_Votes->GroupViewValue);

            // Count
            $this->Count->CntViewValue = $this->Count->CntValue;
            $this->Count->CntViewValue = FormatNumber($this->Count->CntViewValue, $this->Count->formatPattern());
            $this->Count->ViewCustomAttributes = "";
            $this->Count->CellAttrs["class"] = ($this->RowTotalType == ROWTOTAL_PAGE || $this->RowTotalType == ROWTOTAL_GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;

            // Submission_ID
            $this->Submission_ID->HrefValue = "";

            // Votes
            $this->_Votes->HrefValue = "";
            ...
            $this->Submission_ID->GroupViewValue = $this->Submission_ID->groupValue();
            $this->Submission_ID->GroupViewValue = FormatNumber($this->Submission_ID->GroupViewValue, $this->Submission_ID->formatPattern());
            $this->Submission_ID->CellCssClass = "ew-rpt-grp-field-1";
            $this->Submission_ID->ViewCustomAttributes = "";
            $this->Submission_ID->GroupViewValue = DisplayGroupValue($this->Submission_ID, $this->Submission_ID->GroupViewValue);
            if (!$this->Submission_ID->LevelBreak) {
                $this->Submission_ID->GroupViewValue = "";
            } else {
                $this->Submission_ID->LevelBreak = false;
            }
            ...
            $this->_Votes->CellCssClass = "ew-rpt-grp-field-2";
            $this->_Votes->ViewCustomAttributes = "";
            $this->_Votes->GroupViewValue = DisplayGroupValue($this->_Votes, $this->_Votes->GroupViewValue);
            if (!$this->_Votes->LevelBreak) {
                $this->_Votes->GroupViewValue = "";
            } else {
                $this->_Votes->LevelBreak = false;
            }

            // Title
            $this->_Title->ViewValue = $this->_Title->CurrentValue;
            $this->_Title->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            $this->Count->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            $this->Count->ViewCustomAttributes = "";
            ...
            // Votes
            $this->_Votes->LinkCustomAttributes = "";
            $this->_Votes->HrefValue = "";
            $this->_Votes->TooltipValue = "";
        ...
        if ($this->RowType == ROWTYPE_TOTAL) {
            // Submission_ID
            $currentValue = $this->Submission_ID->GroupViewValue;
            $viewValue = &$this->Submission_ID->GroupViewValue;
            $viewAttrs = &$this->Submission_ID->ViewAttrs;
            $cellAttrs = &$this->Submission_ID->CellAttrs;
            $hrefValue = &$this->Submission_ID->HrefValue;
            $linkAttrs = &$this->Submission_ID->LinkAttrs;
            $this->cellRendered($this->Submission_ID, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // Votes
            $currentValue = $this->_Votes->GroupViewValue;
            $viewValue = &$this->_Votes->GroupViewValue;
            $viewAttrs = &$this->_Votes->ViewAttrs;
            $cellAttrs = &$this->_Votes->CellAttrs;
            $hrefValue = &$this->_Votes->HrefValue;
            $linkAttrs = &$this->_Votes->LinkAttrs;
            $this->cellRendered($this->_Votes, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // Count
            $currentValue = $this->Count->CntValue;
            $viewValue = &$this->Count->CntViewValue;
            $viewAttrs = &$this->Count->ViewAttrs;
            $cellAttrs = &$this->Count->CellAttrs;
            $hrefValue = &$this->Count->HrefValue;
            $linkAttrs = &$this->Count->LinkAttrs;
            $this->cellRendered($this->Count, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        } else {
            // Submission_ID
            $currentValue = $this->Submission_ID->groupValue();
            $viewValue = &$this->Submission_ID->GroupViewValue;
            $viewAttrs = &$this->Submission_ID->ViewAttrs;
            $cellAttrs = &$this->Submission_ID->CellAttrs;
            $hrefValue = &$this->Submission_ID->HrefValue;
            ...
            // Votes
            $currentValue = $this->_Votes->groupValue();
            $viewValue = &$this->_Votes->GroupViewValue;
            $viewAttrs = &$this->_Votes->ViewAttrs;
            $cellAttrs = &$this->_Votes->CellAttrs;
            $hrefValue = &$this->_Votes->HrefValue;
            ...
            // Title
            $currentValue = $this->_Title->CurrentValue;
            $viewValue = &$this->_Title->ViewValue;
            $viewAttrs = &$this->_Title->ViewAttrs;
            $cellAttrs = &$this->_Title->CellAttrs;
            $hrefValue = &$this->_Title->HrefValue;
            $linkAttrs = &$this->_Title->LinkAttrs;
            $this->cellRendered($this->_Title, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
...
// Get export HTML tag
    ...
    {
        global $Language, $Security; // added $Security by Masino Sinaga, September 25, 2020
...
// Set up other options
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fVotingsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fVotingsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    protected function setupDisplayGroups()
    {
		...
		global $Language;
        $wrk = Get(Config("TABLE_GROUP_PER_PAGE"), "");
    ...
    protected function getSort()
    {
        ...
        // Check for an Order parameter
            ...
            $this->updateSort($this->_Votes); // Votes
        ...
        if ($this->getOrderBy() == "") {
            $useDefaultSort = true;
            if ($this->_Title->getSort() != "") {
                $useDefaultSort = false;
            }
            if ($useDefaultSort) {
                $this->_Title->setSort("ASC");
                $this->setOrderBy("`Title` ASC");
            }
        }
