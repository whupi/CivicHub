 ...
 * Page class
 */
class SubmissionMonitorGrid extends SubmissionMonitor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubmissionMonitorGrid";
    ...
    public $FormName = "fsubmission_monitorgrid";
...
// Use layout
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Monitor_ID->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_Taskings") && $CurrentForm->hasValue("o_Taskings") && $this->Taskings->CurrentValue != $this->Taskings->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_Start_Date") && $CurrentForm->hasValue("o_Start_Date") && $this->Start_Date->CurrentValue != $this->Start_Date->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_Finish_Date") && $CurrentForm->hasValue("o_Finish_Date") && $this->Finish_Date->CurrentValue != $this->Finish_Date->DefaultValue) {
            return false;
        }
    ...
    {
        $this->Monitor_ID->clearErrorMessage();
        $this->Status->clearErrorMessage();
        $this->Taskings->clearErrorMessage();
        $this->Start_Date->clearErrorMessage();
        $this->Finish_Date->clearErrorMessage();
        $this->Uploads->clearErrorMessage();
        $this->Updated_Username->clearErrorMessage();
    }
...
// Get upload files
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->Updated_Username->DefaultValue = CurrentUserID();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Monitor_ID->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_Taskings")) {
            $this->Taskings->setOldValue($CurrentForm->getValue("o_Taskings"));
        }
        ...
        if ($CurrentForm->hasValue("o_Finish_Date")) {
            $this->Finish_Date->setOldValue($CurrentForm->getValue("o_Finish_Date"));
        }

        // Check field name 'Updated_Username' first before field var 'x_Updated_Username'
        $val = $CurrentForm->hasValue("Updated_Username") ? $CurrentForm->getValue("Updated_Username") : $CurrentForm->getValue("x_Updated_Username");
        if (!$this->Updated_Username->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Updated_Username->Visible = false; // Disable update for API request
            } else {
                $this->Updated_Username->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_Updated_Username")) {
            $this->Updated_Username->setOldValue($CurrentForm->getValue("o_Updated_Username"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->Finish_Date->CurrentValue = UnFormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern());
        $this->Updated_Username->CurrentValue = $this->Updated_Username->FormValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if ($this->Updated_Username->Required) {
            if (!$this->Updated_Username->IsDetailKey && EmptyValue($this->Updated_Username->FormValue)) {
                $this->Updated_Username->addErrorMessage(str_replace("%s", $this->Updated_Username->caption(), $this->Updated_Username->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // Updated_Username
        $this->Updated_Username->CurrentValue = CurrentUserName();
        $this->Updated_Username->setDbValueDef($rsnew, $this->Updated_Username->CurrentValue, null);
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "submission") {
            $this->Submission_ID->CurrentValue = $this->Submission_ID->getSessionValue();
        }
        ...
        // Taskings
        $this->Taskings->setDbValueDef($rsnew, $this->Taskings->CurrentValue, null, false);
        ...
        // Finish_Date
        $this->Finish_Date->setDbValueDef($rsnew, UnFormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern()), null, false);
        ...
        if ($this->Submission_ID->getSessionValue() != "") {
            $rsnew['Submission_ID'] = $this->Submission_ID->getSessionValue();
        }
        if ($this->Uploads->Visible && !$this->Uploads->Upload->KeepFile) {
            $oldFiles = EmptyValue($this->Uploads->Upload->DbValue) ? [] : explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->Uploads->htmlDecode(strval($this->Uploads->Upload->DbValue)));
            if (!EmptyValue($this->Uploads->Upload->FileName)) {
