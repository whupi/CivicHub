...
class UserlevelsView extends Userlevels
{
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("User_Level_ID") ?? Route("User_Level_ID")) !== null) {
            $this->RecKey["User_Level_ID"] = $keyValue;
        }
...
// Is modal
        ...
        if ($this->isPageRequest()) { // Validate request
            ...
            } elseif (Post("User_Level_ID") !== null) {
                $this->User_Level_ID->setFormValue(Post("User_Level_ID"));
    ...
    protected function setupOtherOptions()
    {
        ...
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("users", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("userslist?" . Config("TABLE_SHOW_MASTER") . "=userlevels&" . GetForeignKeyUrl("fk_User_Level_ID", $this->User_Level_ID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("UsersGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'userlevels')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=users"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "users";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'userlevels')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=users"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "users";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'userlevels')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=users"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
        ...
        $item->Body = $body;
        $item->Visible = $Security->allowList(CurrentProjectID() . 'users');
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("users", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->User_Level->IsDetailKey = true;
                    $detailPageObj->User_Level->CurrentValue = $this->User_Level_ID->CurrentValue;
                    $detailPageObj->User_Level->setSessionValue($detailPageObj->User_Level->CurrentValue);
                }
            }
        }
    }
