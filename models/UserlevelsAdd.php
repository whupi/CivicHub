...
// Get upload files
    ...
    public function renderRow()
    {
        ...
        // User_Level_Name
        $this->User_Level_Name->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->User_Level_Name->PlaceHolder = RemoveHtml($this->User_Level_Name->caption());
    ...
    protected function addRow($rsold = null)
    {
        ...
        if (trim($this->User_Level_Name->CurrentValue) == "") {
            $this->setFailureMessage($Language->phrase("MissingUserLevelName"));
        } elseif (in_array(strtolower(trim($this->User_Level_Name->CurrentValue)), ["anonymous", "administrator", "default"])) {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("UsersGrid");
            if (in_array("users", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->User_Level->setSessionValue($this->User_Level_ID->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "users"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->User_Level->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
