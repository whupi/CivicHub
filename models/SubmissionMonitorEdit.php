 ...
 * Page class
 */
class SubmissionMonitorEdit extends SubmissionMonitor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubmissionMonitorEdit";
...
// Is modal
        ...
        $this->Organisations->setVisibility();
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Monitor_ID") ?? Key(0) ?? Route(2)) !== null) {
                $this->Monitor_ID->setQueryStringValue($keyValue);
                $this->Monitor_ID->setOldValue($this->Monitor_ID->QueryStringValue);
            } elseif (Post("Monitor_ID") !== null) {
                $this->Monitor_ID->setFormValue(Post("Monitor_ID"));
                $this->Monitor_ID->setOldValue($this->Monitor_ID->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Monitor_ID") ?? Route("Monitor_ID")) !== null) {
                    $this->Monitor_ID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Monitor_ID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("submissionmonitorlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "submissionmonitorlist") {
...
// Get upload files
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Monitor_ID' first before field var 'x_Monitor_ID'
        $val = $CurrentForm->hasValue("Monitor_ID") ? $CurrentForm->getValue("Monitor_ID") : $CurrentForm->getValue("x_Monitor_ID");
        if (!$this->Monitor_ID->IsDetailKey) {
            $this->Monitor_ID->setFormValue($val);
        }
        ...
        if (!$this->Submission_ID->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Submission_ID->Visible = false; // Disable update for API request
            } else {
        ...
        // Check field name 'Taskings' first before field var 'x_Taskings'
        $val = $CurrentForm->hasValue("Taskings") ? $CurrentForm->getValue("Taskings") : $CurrentForm->getValue("x_Taskings");
        if (!$this->Taskings->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Taskings->Visible = false; // Disable update for API request
            } else {
                $this->Taskings->setFormValue($val);
            }
        }

        // Check field name 'Organisations' first before field var 'x_Organisations'
        $val = $CurrentForm->hasValue("Organisations") ? $CurrentForm->getValue("Organisations") : $CurrentForm->getValue("x_Organisations");
        if (!$this->Organisations->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Organisations->Visible = false; // Disable update for API request
            } else {
                $this->Organisations->setFormValue($val);
            }
        }
        ...
        // Check field name 'Finish_Date' first before field var 'x_Finish_Date'
        $val = $CurrentForm->hasValue("Finish_Date") ? $CurrentForm->getValue("Finish_Date") : $CurrentForm->getValue("x_Finish_Date");
        if (!$this->Finish_Date->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Finish_Date->Visible = false; // Disable update for API request
            } else {
                $this->Finish_Date->setFormValue($val, true, $validate);
            }
            $this->Finish_Date->CurrentValue = UnFormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern());
        }
    ...
    {
        global $CurrentForm;
        $this->Monitor_ID->CurrentValue = $this->Monitor_ID->FormValue;
        $this->Submission_ID->CurrentValue = $this->Submission_ID->FormValue;
        $this->Status->CurrentValue = $this->Status->FormValue;
        $this->Taskings->CurrentValue = $this->Taskings->FormValue;
        $this->Organisations->CurrentValue = $this->Organisations->FormValue;
        $this->Start_Date->CurrentValue = $this->Start_Date->FormValue;
        $this->Start_Date->CurrentValue = UnFormatDateTime($this->Start_Date->CurrentValue, $this->Start_Date->formatPattern());
        $this->Finish_Date->CurrentValue = $this->Finish_Date->FormValue;
        $this->Finish_Date->CurrentValue = UnFormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern());
    }
    ...
    public function renderRow()
    {
        ...
        // Monitor_ID
        $this->Monitor_ID->RowCssClass = "row";
        ...
        // Taskings
        $this->Taskings->RowCssClass = "row";

        // Organisations
        $this->Organisations->RowCssClass = "row";

        // Start_Date
        $this->Start_Date->RowCssClass = "row";

        // Finish_Date
        $this->Finish_Date->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->Submission_ID->getSessionValue() != "") {
                ...
                $curVal = trim(strval($this->Submission_ID->CurrentValue));
                if ($curVal != "") {
                    $this->Submission_ID->ViewValue = $this->Submission_ID->lookupCacheOption($curVal);
                } else {
                    $this->Submission_ID->ViewValue = $this->Submission_ID->Lookup !== null && is_array($this->Submission_ID->lookupOptions()) ? $curVal : null;
                }
                if ($this->Submission_ID->ViewValue !== null) { // Load from cache
                    $this->Submission_ID->EditValue = array_values($this->Submission_ID->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`Submission_ID`" . SearchString("=", $this->Submission_ID->CurrentValue, DATATYPE_NUMBER, "");
                    }
                    $sqlWrk = $this->Submission_ID->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    $arwrk = $rswrk;
                    $this->Submission_ID->EditValue = $arwrk;
                }
            ...
            $this->Taskings->EditValue = HtmlEncode($this->Taskings->CurrentValue);
            ...
            $curVal = trim(strval($this->Organisations->CurrentValue));
            if ($curVal != "") {
                $this->Organisations->ViewValue = $this->Organisations->lookupCacheOption($curVal);
            } else {
                $this->Organisations->ViewValue = $this->Organisations->Lookup !== null && is_array($this->Organisations->lookupOptions()) ? $curVal : null;
            }
            if ($this->Organisations->ViewValue !== null) { // Load from cache
                $this->Organisations->EditValue = array_values($this->Organisations->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                ...
                $sqlWrk = $this->Organisations->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->Organisations->EditValue = $arwrk;
            }
            ...
            $this->Finish_Date->EditValue = HtmlEncode(FormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->Monitor_ID->Required) {
            if (!$this->Monitor_ID->IsDetailKey && EmptyValue($this->Monitor_ID->FormValue)) {
                $this->Monitor_ID->addErrorMessage(str_replace("%s", $this->Monitor_ID->caption(), $this->Monitor_ID->RequiredErrorMessage));
            }
        }
        ...
        if ($this->Taskings->Required) {
            if (!$this->Taskings->IsDetailKey && EmptyValue($this->Taskings->FormValue)) {
                $this->Taskings->addErrorMessage(str_replace("%s", $this->Taskings->caption(), $this->Taskings->RequiredErrorMessage));
            }
        }
        if ($this->Organisations->Required) {
            if ($this->Organisations->FormValue == "") {
                $this->Organisations->addErrorMessage(str_replace("%s", $this->Organisations->caption(), $this->Organisations->RequiredErrorMessage));
            }
        }
        ...
        if ($this->Finish_Date->Required) {
            if (!$this->Finish_Date->IsDetailKey && EmptyValue($this->Finish_Date->FormValue)) {
                $this->Finish_Date->addErrorMessage(str_replace("%s", $this->Finish_Date->caption(), $this->Finish_Date->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->Finish_Date->FormValue, $this->Finish_Date->formatPattern())) {
            $this->Finish_Date->addErrorMessage($this->Finish_Date->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // Submission_ID
        if ($this->Submission_ID->getSessionValue() != "") {
            $this->Submission_ID->ReadOnly = true;
        }
        ...
        // Taskings
        $this->Taskings->setDbValueDef($rsnew, $this->Taskings->CurrentValue, null, $this->Taskings->ReadOnly);

        // Organisations
        $this->Organisations->setDbValueDef($rsnew, $this->Organisations->CurrentValue, null, $this->Organisations->ReadOnly);
        ...
        // Finish_Date
        $this->Finish_Date->setDbValueDef($rsnew, UnFormatDateTime($this->Finish_Date->CurrentValue, $this->Finish_Date->formatPattern()), null, $this->Finish_Date->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("submissionmonitorlist"), "", $this->TableVar, true);
        $pageId = "edit";
