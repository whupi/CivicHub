...
/**
 * Table class for submission_vote
 */
class SubmissionVote extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'submission_vote';
        $this->TableName = 'submission_vote';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`submission_vote`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Vote->InputTextType = "text";
        $this->Vote->Nullable = false; // NOT NULL field
        $this->Vote->Required = true; // Required field
        $this->Vote->Lookup = new Lookup('Vote', 'submission_vote', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
        $this->Vote->OptionCount = 2;
        $this->Fields['Vote'] = &$this->Vote;
        ...
        $this->Updated_Username->IsPrimaryKey = true; // Primary key field
        $this->Updated_Username->Nullable = false; // NOT NULL field
        $this->Fields['Updated_Username'] = &$this->Updated_Username;

        // Updated_Date
        $this->Updated_Date = new DbField(
            'submission_vote',
            'submission_vote',
            'x_Updated_Date',
            'Updated_Date',
            '`Updated_Date`',
            CastDateFieldForLike("`Updated_Date`", 0, "DB"),
            135,
            19,
            0,
            false,
        ...
        $this->Updated_Date->InputTextType = "text";
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`submission_vote`";
    }
    ...
    public function getSqlSelectList() // Select for List page
    {
        ...
        $from = "(SELECT *, (SELECT `Title` FROM `submission_view` `TMP_LOOKUPTABLE` WHERE `TMP_LOOKUPTABLE`.`Submission_ID` = `submission_vote`.`Submission_ID` LIMIT 1) AS `EV__Submission_ID` FROM `submission_vote`)";
    ...
    protected function useVirtualFields()
    {
        ...
        if (ContainsString($orderBy, " " . $this->Submission_ID->VirtualExpression . " ")) {
            return true;
        }
        return false;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            ...
            if (array_key_exists('Updated_Username', $rs)) {
                AddFilter($where, QuotedName('Updated_Username', $this->Dbid) . '=' . QuotedValue($rs['Updated_Username'], $this->Updated_Username->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        return "`Submission_ID` = @Submission_ID@ AND `Updated_Username` = '@Updated_Username@'";
    }
    ...
    public function getKey($current = false)
    {
        ...
        $val = $current ? $this->Updated_Username->CurrentValue : $this->Updated_Username->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->Updated_Username->CurrentValue = $keys[1];
            } else {
                $this->Updated_Username->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Updated_Username', $row) ? $row['Updated_Username'] : null;
        } else {
            $val = $this->Updated_Username->OldValue !== null ? $this->Updated_Username->OldValue : $this->Updated_Username->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@Updated_Username@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("submissionvotelist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "submissionvoteedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "submissionvoteadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "SubmissionVoteView";
            case Config("API_ADD_ACTION"):
                return "SubmissionVoteAdd";
            case Config("API_EDIT_ACTION"):
                return "SubmissionVoteEdit";
            case Config("API_DELETE_ACTION"):
                return "SubmissionVoteDelete";
            case Config("API_LIST_ACTION"):
                return "SubmissionVoteList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("submissionvoteview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("submissionvoteview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "submissionvoteadd?" . $this->getUrlParm($parm);
        } else {
            $url = "submissionvoteadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("submissionvoteedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("submissionvoteadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("submissionvotedelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "Voting" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_Submission_ID", $this->Submission_ID->CurrentValue);
        }
        if ($this->getCurrentMasterTable() == "vote_tally" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_Submission_ID", $this->Submission_ID->CurrentValue);
        }
        return $url;
    ...
    public function keyToJson($htmlEncode = false)
    {
        ...
        $json .= ",\"Updated_Username\":" . JsonEncode($this->Updated_Username->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->Updated_Username->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->Updated_Username->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("Updated_Username") ?? Route("Updated_Username")) !== null) {
                $arKey[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Submission_ID->CurrentValue = $key[0];
            } else {
                $this->Submission_ID->OldValue = $key[0];
            }
            if ($setCurrent) {
                $this->Updated_Username->CurrentValue = $key[1];
            } else {
                $this->Updated_Username->OldValue = $key[1];
            }
    ...
    public function getUserIDSubquery(&$fld, &$masterfld)
    {
        global $UserTable;
        $wrk = "";
        $sql = "SELECT " . $masterfld->Expression . " FROM `submission_vote`";
